...
from torch_geometric.datasets import Reddit, PPI, Yelp, AmazonProducts, Flickr, Planetoid
...
# get the simplified computation graph
def transfer_graph_contribution(aggregated_pairs, depths, num_v, num_neighbors, idxs, data, maxDepth):
    edge_index0 = data.edge_index[0]
    edge_index1 = data.edge_index[1]
    x = data.x
    v = num_v
...
for data in dataset:
    ...
    newids, ranges, ranges_index, opt_edge_index, x, num_neighbors, idxs, redundancy_eliminated = \
        transfer_graph(H_sorted, num_v, num_neighbors, idxs, data, 4, int(data.x.shape[0]/4))
    t2 = time.time()
    total_construct_time += (t2 - t1)
    print("constructing HAG cost: ", t2 - t1)
    y = data.y
    ...
    t3 = time.time()
    aggregated_pairs, depths = get_aggregated_pairs(contribution_data, 4, int(data.x.shape[0]/4), 0.8)
    t4 = time.time()
    total_extra_time += t4 - t3
    print("extra time: ", t4 - t3)
    H_sorted, num_v, num_neighbors, idxs = get_redundancy_heap(contribution_data.edge_index, contribution_data.x)
    newids, ranges, ranges_index, opt_edge_index, x, num_neighbors, idxs, redundancy_eliminated = \
        transfer_graph_contribution(aggregated_pairs, depths, num_v, num_neighbors, idxs, contribution_data, 4)
    t5 = time.time()
...
print("hag time: {}, hag with contribution time:{}, contribution extra time: {}"
